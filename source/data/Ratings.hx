...
class Ratings
{
    ...
    public static function GenerateLetterRank(accuracy:Float) // generate a letter ranking
    {
        ...
        if (PlayState.scoreData.misses == 0 && PlayState.scoreData.bads == 0 && PlayState.scoreData.shits == 0 && PlayState.scoreData.goods == 0) // Marvelous (SICK) Full Combo
            ranking = "(MFC)";
        else if (PlayState.scoreData.misses == 0 && PlayState.scoreData.bads == 0 && PlayState.scoreData.shits == 0 && PlayState.scoreData.goods >= 1) // Good Full Combo (Nothing but Goods & Sicks)
            ranking = "(GFC)";
        else if (PlayState.scoreData.misses == 0) // Regular FC
            ranking = "(FC)";
        else if (PlayState.scoreData.misses < 10) // Single Digit Combo Breaks
            ranking = "(SDCB)";
        else
            ranking = "(Clear)";
