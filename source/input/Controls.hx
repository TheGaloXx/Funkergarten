...
// Wsp sanco here, i rewrote the input of my engine for cleaner and better code
// galo asked me to add gamepad support and so we decided to rewrite most of the controls code too (cuz its too long)
// so here it is, reading the comments should probably resolve any question you have about this cooked code but if you need help hmu 
...
class Controls
{
	...
	public static var noteStates:Array<String> = [Type.getClassName(PlayState)];
	...
	public static function dispatchPressed(action:ActionType)
	{
		...
		if ((cast(FlxG.state, MusicBeatState).controls == null && (!FlxG.state.active || !FlxG.state.persistentUpdate)) || (actor == NONE))
			return;

		Reflect.setField(Reflect.field(cast(FlxG.state, MusicBeatState).controls, action), "state", ActionState.PRESSED);
	...
	public static function dispatchReleased(action:ActionType)
	{
		...
		Reflect.setField(Reflect.field(cast(FlxG.state, MusicBeatState).controls, action), "state", ActionState.RELEASED);
