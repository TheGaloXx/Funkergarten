...
class Strum extends flixel.FlxSprite
{
    ...
    {
        super(0, (KadeEngineData.settings.data.downscroll ? FlxG.height - 165 : 40));
		...
		if (!isPlayer && KadeEngineData.settings.data.middlescroll)
		{
		...
		x += (Note.swagWidth * Math.abs(i) + 100 + ((FlxG.width / 2) * (isPlayer ? 1 : 0))) - (KadeEngineData.settings.data.middlescroll ? 275 : 0);
    }
