...
class CreditsState extends MusicBeatState
{
	...
	private var grpCredits:Array<DialogueBox.IconBox> = [];
	...
	override function create()
	{
		...
		{
			var sprite = new DialogueBox.IconBox(credits[i].devName, credits[i].devName, credits[i].color, false);
	...
	override function update(elapsed:Float)
	{
		...
		for (i in 0...grpCredits.length)
			if (i != curSelected)
				grpCredits[i].x = flixel.math.FlxMath.lerp(DialogueBox.IconBox.daX, grpCredits[i].x, 0.5 * (elapsed * 30));
